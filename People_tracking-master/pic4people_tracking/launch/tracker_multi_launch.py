...
pkg_share = get_package_share_directory('pic4people_tracking')

params_center = os.path.join(pkg_share, 'params', 'params_center.yaml')
params_left   = os.path.join(pkg_share, 'params', 'params_left.yaml')
params_right  = os.path.join(pkg_share, 'params', 'params_right.yaml')
...
def generate_launch_description():
    ...
    # include main nodes (tracker for each camera)
    tracker_center = Node(
    package='pic4people_tracking',
    executable='tracker_ReID',
    name='tracker_center',
    parameters=[params_center, {'use_sim_time': True}]
    )
    ...
    tracker_left = Node(
        ...
        executable='tracker_ReID',
        name='tracker_left',
        parameters=[params_left, {'use_sim_time': True}]
    )
    ...
    tracker_right = Node(
        ...
        executable='tracker_ReID',
        name='tracker_right',
        parameters=[params_right, {'use_sim_time': True}]
    )
    
    fusion_node = Node(
        package='pic4people_tracking',
        executable='fusion_node_ReID',
        name='fusion_node',
        output='screen',
