...
from pic4people_tracking.StrongSORT.utils.iou import iou_batch
...
def _cosine_distance(a, b, data_is_normalized=False):
    ...
    if a.shape[0] == 1:
        ret = 1.0 - np.sum(a * b, axis=1)  # Faster than `np.dot` for a single row
    elif b.shape[0] == 1:
        ret = 1.0 - np.sum(a * b, axis=1)
    else:
        ret = 1.0 - np.dot(a, b.T)
    return ret
...
def _nn_cosine_distance(x, y):
    ...
    ----------
    x : ndarray
        A matrix of N row-vectors (sample points). (Past samples of one track)
    y : ndarray
        A matrix of M row-vectors (query points). (Detections)
    Returns
    -------
    ndarray
        A vector of length M that contains for each entry in `y` the
...
class NearestNeighborDistanceMetric(object):
    ...
    def distance(self, features, targets):
        ...
        print("Appearance cost matrix: ", cost_matrix)
        return cost_matrix
