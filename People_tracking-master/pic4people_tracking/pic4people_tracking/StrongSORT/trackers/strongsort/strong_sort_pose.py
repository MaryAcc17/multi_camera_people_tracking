...
from pic4people_tracking.StrongSORT.trackers.strongsort.sort.tracker_pose import Tracker
...
class StrongSORT(object):
    ...
    def update(self, dets: np.ndarray, img: np.ndarray, embs: np.ndarray = None, dt: float = 1./22) -> np.ndarray:
        assert isinstance(
            dets, np.ndarray
        ...
        assert (
            dets.shape[1] == 9
        ), "Unsupported 'dets' 2nd dimension lenght, valid lenghts is 9"
        ...
        yaws = dets[:, 7]
        confs = dets[:, 8]
        clss = dets[:,9].astype("int") # 0: Person
        det_ind = dets[:, 10].astype("int")
        print(f"det yaws: {yaws}")
        ...
        self.tracker.update(detections, yaws)
    ...
    def estimate(self, dt):
        ...
        for track in self.tracker.tracks:
            print("track id: ",track.id)
            if not (track.is_tentative() or track.is_confirmed()):
                continue
    ...
    def get_yaws(self):
        """
        return the yaw angle of the currently active trackers. Assumed to be called after update method.
        """
        ret = []
        ...
        for track in self.tracker.tracks:
            ...
            yaw = track.yaw_mean[0][0]
            id = track.id
            det_ind = track.det_ind
    
            ret.append(
                np.concatenate(([yaw], [id], [det_ind])).reshape(1, -1) # [conf], [cls], 
            )
