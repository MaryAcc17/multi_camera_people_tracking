...
from pic4people_tracking_msgs.msg import TrackedPeople, TrackedPerson
...
class PeopleTracker(Node):
    def __init__(self):
        ...
        self.yolo_ov = yolov8pose_OpenVINO(
            model_filepath=self.yolo_model,
    ...
    def define_publishers(self):
        # Publish local tracks on the same topic names as before
        # (/people_center, /people_left, /people_right), but now using the
        # custom TrackedPeople message. Each TrackedPerson contains position,
        # velocity, yaw, reliability and the Re-ID embedding in the same message.
        self.tracked_people_publisher = self.create_publisher(
            TrackedPeople,
            self.tracked_people_topic,
            10
        )
        ...
        # - whether an embedding was available for each published local track
        self.local_debug_pub = self.create_publisher(String, '/local_tracker_debug', 10)

        self.get_logger().info(
            f"TrackedPeople topic: {self.tracked_people_topic} "
            "(custom message with embedded Re-ID features)"
        )
    ...
    def define_parameters(self):
        ...
        # Fallback parameters.
        # The fallback is intentionally conservative because it estimates 3D position
        # from the bbox-center depth patch when the keypoint-based estimate is weak.
        # This prevents accepting wall/background depth as a valid person position.
        self.declare_parameter('fallback_min_valid_keypoints', 3)
        self.declare_parameter('fallback_min_seed_keypoints', 2)
        self.declare_parameter('fallback_min_box_score', 0.75)
        self.declare_parameter('fallback_min_box_width', 25)
        self.declare_parameter('fallback_min_box_height', 50)
        self.declare_parameter('fallback_patch_radius', 7)
        self.declare_parameter('fallback_min_valid_points', 20)
        self.declare_parameter('fallback_min_valid_fraction', 0.40)
        self.declare_parameter('fallback_max_depth_std', 0.15)
        self.declare_parameter('fallback_max_depth_jump_from_keypoints', 0.60)
        ...
        self.fallback_min_seed_keypoints = self.get_parameter('fallback_min_seed_keypoints').value
        ...
        self.fallback_max_depth_jump_from_keypoints = self.get_parameter('fallback_max_depth_jump_from_keypoints').value
        ...
        self.get_logger().info(
            f"Fallback: enabled={self.fallback_enable}, "
            f"min_seed_keypoints={self.fallback_min_seed_keypoints}, "
            f"min_box_score={self.fallback_min_box_score}, "
            f"min_valid_points={self.fallback_min_valid_points}, "
            f"min_valid_fraction={self.fallback_min_valid_fraction}, "
            f"max_depth_std={self.fallback_max_depth_std}, "
            f"max_depth_jump_from_keypoints={self.fallback_max_depth_jump_from_keypoints}"
        )
    ...
    def get_bbox_center_fallback(self, xyxy, score, valid_points_from_keypoints=None):
        """
        ...
        # 9. Consistency check with the few valid keypoints already available.
        # If the bbox-center patch depth is very different from the keypoint depth,
        # the center is probably on the background/wall and must be rejected.
        if valid_points_from_keypoints is not None and len(valid_points_from_keypoints) > 0:
            seed_points = np.asarray(valid_points_from_keypoints, dtype=np.float32)
            seed_depth = float(np.median(seed_points[:, 2]))
            depth_jump = abs(float(pos3D[2]) - seed_depth)
            if depth_jump > self.fallback_max_depth_jump_from_keypoints:
                self.get_logger().warn(
                    f"[FALLBACK REJECT] bbox-center depth inconsistent with seed keypoints: "
                    f"fallback_z={float(pos3D[2]):.3f}, seed_z={seed_depth:.3f}, "
                    f"jump={depth_jump:.3f} > max={self.fallback_max_depth_jump_from_keypoints:.3f}"
                )
                return None
    ...
    def get_3Dpositions(self, xyxys, keypoints, boxes_with_scores=None):
        ...
        for det_idx, (xyxy, kpoints) in enumerate(zip(xyxys, keypoints)):
            ...
            # The fallback is allowed only when there is at least a small
            # keypoint support. This prevents creating a 3D person position
            # from the bbox center alone when YOLO/keypoints are very uncertain.
            elif self.fallback_enable:
                if len(valid_points) < self.fallback_min_seed_keypoints:
                    debug_row.update({
                        "method": "dropped",
                        "reason": "not_enough_seed_keypoints_for_fallback",
                    })
                    self.get_logger().warn(
                        f"[3D DEBUG][DROP] det={det_idx}: fallback blocked because "
                        f"valid keypoints={len(valid_points)} < "
                        f"min_seed_keypoints={self.fallback_min_seed_keypoints}"
                    )
                else:
                    fallback = self.get_bbox_center_fallback(
                        xyxy,
                        score if np.isfinite(score) else 1.0,
                        valid_points_from_keypoints=valid_points,
                    )
                    ...
                    if fallback is not None:
                        ...
                        debug_row.update({
                            ...
                            "reason": "ok",
                            "fallback_seed_keypoints": int(len(valid_points)),
                            "camera_x": float(pos3D[0]),
                            "camera_y": float(pos3D[1]),
                            "camera_z": float(pos3D[2]),
                        })
    ...
    def _camera_name_from_topic(self):
        """
        Infer the camera name from the configured output topic.
        ...
        If the topic does not follow this convention, the full topic suffix is
        still returned as a stable string.
        """
        topic = str(self.tracked_people_topic).strip("/")
        if topic.startswith("people_"):
            return topic.replace("people_", "", 1)
        return topic or str(self.camera_frame)

    def publish_local_tracker_debug(self, tracks=None, yaws=None, velocities=None, reliabilities=None, reid_features_by_id=None):
        """
        ...
        payload = {
            ...
            "message_type": "pic4people_tracking_msgs/TrackedPeople",
            "features_transport": "embedded_in_tracked_people",
            ...
            for track, yaw, vel, rel in zip(tracks, yaws, velocities, reliabilities):
                try:
                    ...
                    payload["tracks"].append({
                        ...
                        "published_by_local_tracker": bool(rel_value >= 0.25),
                        "embedding_available": bool(
                            reid_features_by_id is not None and local_id in reid_features_by_id
                        ),
                        "embedding_dim": int(
                            reid_features_by_id[local_id].size
                        ) if reid_features_by_id is not None and local_id in reid_features_by_id else 0,
                    })
                except Exception as e:
    ...
    def _get_active_reid_features_by_id(self):
        """
        Return the latest smoothed OSNet/Re-ID feature for each currently active
        local StrongSORT track.

        The returned dictionary is keyed by the local StrongSORT ID, i.e. the same
        ID used in the published People name: Person_<id>.
        """
        features_by_id = {}

        try:
            strongsort_tracker = getattr(self.tracker, "tracker", None)
            local_tracks = getattr(strongsort_tracker, "tracks", [])

            for tr in local_tracks:
                if not (tr.is_confirmed() and tr.time_since_update < 1):
                    continue

                if not hasattr(tr, "features") or len(tr.features) == 0:
                    self.get_logger().warn(
                        f"[REID FEATURE SKIP] local ID={int(tr.id)} has no feature cache"
                    )
                    continue

                feat = np.asarray(tr.features[-1], dtype=np.float32).reshape(-1)
                norm = float(np.linalg.norm(feat))
                if not np.isfinite(norm) or norm < 1e-12:
                    self.get_logger().warn(
                        f"[REID FEATURE SKIP] local ID={int(tr.id)} invalid feature norm={norm}"
                    )
                    continue

                feat = feat / norm
                features_by_id[int(tr.id)] = feat

        except Exception as e:
            self.get_logger().warn(f"[REID FEATURE] failed to read StrongSORT features: {e}")
    ...
    def publish_tracking_results(self, tracks, yaws, velocities, reliabilities=[]):
        ...
        count_loop = 0
        reid_features_by_id = self._get_active_reid_features_by_id()
        camera_name = self._camera_name_from_topic()
        ...
        for track, yaw, vel, rel in zip(tracks, yaws, velocities, reliabilities):
            ...
            if rel_value < 0.25:
                self.get_logger().warn(
                    ...
                    f"ID={int(track[7])} rel={rel_value:.3f} < 0.250"
                )
                continue
            ...
            local_id_int = int(ID)
            reid_feat = reid_features_by_id.get(local_id_int, None)

            tracked_person = TrackedPerson()
            tracked_person.camera = camera_name
            tracked_person.local_id = f"Person_{local_id_int}"
            tracked_person.position = Point(
                x=float(pose_t.position.x),
                y=float(pose_t.position.y),
                z=0.0
            )
            tracked_person.velocity = Point(
                x=float(absolute_vel[0]),
                y=float(absolute_vel[1]),
                z=0.0
            )
            tracked_person.yaw = float(yaw_t)
            tracked_person.reliability = float(rel_value)

            if reid_feat is not None:
                tracked_person.embedding = reid_feat.astype(np.float32).tolist()
                self.get_logger().info(
                    f"[REID EMBEDDED] local_id=Person_{local_id_int} "
                    f"dim={reid_feat.size} norm={np.linalg.norm(reid_feat):.3f}"
                )
            else:
                tracked_person.embedding = []
                self.get_logger().warn(
                    f"[REID EMBEDDING MISSING] local_id=Person_{local_id_int}; "
                    "TrackedPerson will be published with empty embedding"
                )
        ...
        self.publish_local_tracker_debug(tracks, yaws, velocities, reliabilities, reid_features_by_id)
