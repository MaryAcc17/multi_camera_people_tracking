...
#include "common_utils/path_handler.hpp"
...
nav_msgs::msg::Path PathHandler::transformGlobalPlan(
  const geometry_msgs::msg::PoseStamped & pose, double max_robot_pose_search_dist)
{
  ...
  if (!nav_2d_utils::transformPose(
      tf_buffer_, global_plan_.header.frame_id, pose, robot_pose, transform_tolerance_))
  {
  ...
  auto transformation_end = std::find_if(
    transformation_begin, global_plan_.poses.end(), [&](const auto & global_plan_pose) {
  ...
  auto transformGlobalPoseToLocal = [&](const auto & global_plan_pose) {
      ...
      if (!nav_2d_utils::transformPose(
          tf_buffer_, costmap_ros_->getGlobalFrameID(), stamped_pose, transformed_pose,
  ...
  if (transformation_begin != global_plan_.poses.begin()) {
    ...
    pruned_plan_.poses.insert(
      pruned_plan_.poses.end(),
...
void PathHandler::resetPlan()
{
  ...
  restored_plan.poses.insert(
    restored_plan.poses.end(),
    std::make_move_iterator(global_plan_.poses.begin()),
...
geometry_msgs::msg::PointStamped PathHandler::getTransformedGoal(
  const double & goal_dist, const nav_msgs::msg::Path & transformed_plan,
  const geometry_msgs::msg::PoseStamped & robot_pose)
{
  ...
  auto goal_pose_it = std::find_if(
    transformed_plan.poses.begin(), transformed_plan.poses.end(), [&](const auto & ps) {
      ...
      // return std::hypot(ps.pose.position.x, ps.pose.position.y) >= goal_dist;
